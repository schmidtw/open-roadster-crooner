...
static int intel_read_data( intel_hex_data_reader_t *reader, struct intel_record *record )
{
    int i;
    int c;
    int status;
     ...
     *   rr - record type
     */
    if( 9 != (*reader->fgets)(reader->data_stream, 10, buffer) ) return -1;
    ...
    for( i = 0; i < record->count; i++ ) {
        ...
        if( 2 != (*reader->fgets)(reader->data_stream, 3, buffer) ) return -3;
    ...
    /* Read the checksum */
    if( 2 != (*reader->fgets)(reader->data_stream, 3, buffer) ) return -5;
    if( 1 != sscanf(buffer, "%02x", &(record->checksum)) ) return -6;

    /* Chomp the [\r]\n */
    c = (*reader->fgetc)( reader->data_stream );
    if( '\r' == c ) {
        c = (*reader->fgetc)( reader->data_stream );
    }
    if( '\n' != c ) {
...
static int intel_parse_line( intel_hex_data_reader_t *reader, struct intel_record *record )
{
    if( 0 != intel_read_data(reader, record) )
        return -1;
...
int16_t *intel_hex_to_buffer( intel_hex_data_reader_t *reader, int max_size, int *usage )
{
    int16_t *memory = NULL;
    int failure = 1;
    ...
    if( (NULL == reader) || (NULL == reader->fgets) ||
        (NULL == reader->fgetc) || (0 >= max_size) )
    {
        fprintf( stderr, "Invalid reader or max_size.\n" );
        goto error;
    }
    ...
    *usage = 0;
    do {
        if( 0 != intel_parse_line(reader, &record) ) {

...
static db_status_t __get_next_item_from_list( generic_node_t ** node, bt_get_t next ) {
    bt_node_t *next_node = bt_get(&(*node)->parent->list.children, &(*node)->node, next);
                        ...
                        const db_level_t level )
{
    ...
    if( NULL != song ) {
        switch( level ) {
            case DL_SONG:
                rv = song->track_number;
                break;
            case DL_ALBUM:
            {
                rv = song->d.parent->index;
                break;
            }
            case DL_ARTIST:
            {
                rv = song->d.parent->parent->index;
                break;
            }
            default:
                break;
                       ...
                       const db_level_t level )
{
    ...
    if(    ( false == rdn.initialized )
        || ( NULL == current_song )
        || ( NULL == bt_get_head(&rdn.root->list.children) ) )
    {
        return DS_FAILURE;
    }
    
    if( NULL == *current_song ) {
        generic_n = __ns_get_head(&rdn.root->list.children);
        while(1) {
            if( NULL == generic_n ) {
                return DS_FAILURE;
            }
            ...
            generic_n = __ns_get_head(&generic_n->list.children);
        }
    ...
    switch( operation ) {
        case DT_NEXT:
            ...
            if( DS_END_OF_LIST == rv ) {
                generic_n = __ns_get_head( &generic_n->parent->list.children );
            }
            break;
        case DT_PREVIOUS:
            rv = __get_next_item_from_list( &generic_n, BT_GET__PREVIOUS );
            if( DS_END_OF_LIST == rv ) {
                generic_n = __ns_get_tail( &generic_n->parent->list.children );
            }
            break;
        default:
            /* DT_RANDOM */
            rv = DS_SUCCESS;
            generic_n = (generic_node_t*)generic_n->parent;
            if( NULL == generic_n->list.children.root ) {
                rv = DS_FAILURE;
            } else {
                generic_n = find_random_song_from_generic(
                        (generic_node_t *)generic_n,
                        generic_n->list.index_songs_start,
                        generic_n->list.index_songs_stop);
            }
            break;
    }
    if( NULL != generic_n ) {
        ...
        uint32_t first_song_index, uint32_t last_song_index )
{
    ...
    {
        generic_n = __ns_find( &generic_n->list.children, &song_index );
    }
    return generic_n;
}
...
int8_t __compare_indexed_general_song_search( __song_index_t* si, generic_node_t *node )
{
    if( si->index < node->list.index_songs_start ) {
        return -1;
    } else if( si->index > node->list.index_songs_stop ) {
        return 1;
    }
    return 0;
}
...
int8_t __compare_indexed_general( generic_node_t *node1, generic_node_t *node2 )
{
    uint32_t range_1 = node1->list.index_songs_start;
    uint32_t range_2 = node2->list.index_songs_start;
